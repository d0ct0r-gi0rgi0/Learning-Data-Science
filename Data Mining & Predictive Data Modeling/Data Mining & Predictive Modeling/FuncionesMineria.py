...
def graficoVcramer(matriz, target):
    ...
    title = 'Comparación del coeficiente V de Cramer de ' + target.name
    plt.title(title)
    plt.show()
...
def crear_data_modelo(datos, var_cont, var_categ, var_interac=[], original_data = None):
    """
    ...
    original_data (DataFrame, opcional): DataFrame original del cual se obtienen todos los valores únicos para no perder variables 'dummie' en la partición 80/20.
    ...
    if original_data is not None:
        names = []
        zeros = [0]*len(datos_v.index.tolist())
        for column in var_categ:
            names = [column + '_' + str(value) for value in original_data[column].unique().tolist()]
            names.sort()
            for name in names[1:]:
                if name not in datos_v.columns.tolist():
                    datos_v[name] = zeros
                    datos_v[name] = datos_v[name].astype('uint8')
...
def lm_forward(varObjCont, datos, var_cont, var_categ, var_interac = [], metodo = 'AIC'):
    ...
    while((dif_metrica > 0) and (len(variables) > 0)):
        ...
        if dif_metrica <= 0:
            ...
            print('')
            print(metodo + ' = ' + str(metrica_inicial))
            print('')
            print('y ~ ' + formula)
            print('')
